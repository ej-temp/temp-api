...
describe('app routes', () => {
  ...
  it('gets the hotest current temperature', async() => {
    const cities = await Location.create([
      { name: 'Portland' },
      { name: 'Cleveland' },
      { name: 'Dayton' },
    ...
    return request(app)
      .get('/api/v1/temperatures/hottest')
      .then(res => {
        expect(res.body).toEqual({
          _id: expect.any(String),
          temp: 83,
  ...
  it('gets the average current temperature', async() => {
    const cities = await Location.create([
      { name: 'Portland' },
      { name: 'Cleveland' },
      { name: 'Dayton' },
    ...
    return request(app)
      .get('/api/v1/temperatures/average')
      .then(res => {
        expect(res.body).toEqual({ avgTemp: expect.any(Number) });
      });
  });
